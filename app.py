    ...
    with col2:
        st.markdown(f"""
            <div class="card model-card">
                ...
                <p>Choose a deep learning model for fracture detection:</p>
                
                <div style="margin-bottom: 1.5rem;">
                    <label for="model-select" style="display: block; margin-bottom: 0.5rem; font-weight: 600;">Select Model:</label>
                    <select id="model-select" style="width: 100%; padding: 0.5rem; border-radius: 4px; border: 1px solid var(--border);">
                ...
                <div class="model-info" style="display: none;">
                    <h4 style="margin-top: 0; margin-bottom: 1rem;">Selected Model Information</h4>
                    
                    <div style="margin-bottom: 1.5rem;">
                        <h5 style="margin: 0 0 0.5rem 0;">Performance Metrics</h5>
                        <div style="margin: 1rem 0;">
                            <div style="display: flex; justify-content: space-between; margin-bottom: 0.3rem;">
                                <span>Accuracy</span>
                                <span class="accuracy-value">92-96%</span>
                            </div>
                            <div style="height: 6px; background: var(--border); border-radius: 3px;">
                                <div class="accuracy-bar" style="width: 94%; height: 100%; background: var(--primary); border-radius: 3px;"></div>
                            </div>
                        </div>
                        ...
                        <div style="height: 6px; background: var(--border); border-radius: 3px; margin-bottom: 1rem;">
        ...
        st.markdown("""
        <script>
        // Get the Streamlit selectbox and our custom select
        const streamlitSelect = document.querySelector('select[aria-label="🧠 Select AI Model"]');
        const customSelect = document.getElementById('model-select');
        const modelInfo = document.querySelector('.model-info');

        // Sync our custom select with Streamlit's selectbox
        if (streamlitSelect && customSelect) {
            // Update custom select when Streamlit select changes
            streamlitSelect.addEventListener('change', function() {
            ...
            // Update Streamlit select when custom select changes
            customSelect.addEventListener('change', function() {
                streamlitSelect.value = this.value;
                
                // Dispatch change event to ensure Streamlit reacts
                const event = new Event('change');
                streamlitSelect.dispatchEvent(event);
            ...
            // Initialize state
            if (streamlitSelect.value) {
                customSelect.value = streamlitSelect.value;
                modelInfo.style.display = 'block';
            }
        }
        </script>
