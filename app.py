...
# Function to generate the PDF report using matplotlib
def generate_pdf(fracture_status, confidence_percent, model_name):
    # Create a byte stream to save the PDF in memory
    buffer = BytesIO()

    # Create a matplotlib figure and save it to the buffer
    fig, ax = plt.subplots(figsize=(8.5, 11))  # A4 size
    ax.axis('off')

    # Add text for the report
    ax.text(0.5, 1, "BoneScan AI - Fracture Detection Report", ha='center', va='top', fontsize=16, fontweight='bold')
    ax.text(0.5, 0.95, f"Date & Time: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ha='center', va='top', fontsize=10)
    ax.text(0.5, 0.90, f"Model: {model_name}", ha='center', va='top', fontsize=12)
    ax.text(0.5, 0.85, f"Fracture Status: {fracture_status}", ha='center', va='top', fontsize=12)
    ax.text(0.5, 0.80, f"Confidence: {confidence_percent:.1f}%", ha='center', va='top', fontsize=12)
    ax.text(0.5, 0.75, "Disclaimer: This tool is for research purposes only. Always consult a qualified healthcare professional for medical diagnosis.", ha='center', va='top', fontsize=8)
    ...
    if uploaded_file:
        try:
            ...
            with st.spinner("🔍 Analyzing image..."):
                ...
                pdf_file = generate_pdf(result, confidence_percent, selected_model_name)
