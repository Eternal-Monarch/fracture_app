...
with st.sidebar:
    st.markdown("""
        <div class="sidebar-header">
            <img src="https://www.nitm.ac.in/cygnus/nitmeghalaya/ckfinder/userfiles/images/NITM.gif" width="120">
            <h1 style='font-size: 1.5rem; margin-bottom: 0.5rem; color: var(--primary);'>BoneScan AI</h1>
            <p style='font-size: 0.9rem; opacity: 0.8; margin-top: 0;'>Medical Imaging & Prescription System</p>
        </div>
        
        <div class="sidebar-section">
            <div class="sidebar-section-title">Model Selection</div>
            <select class="sidebar-select" id="sidebar-model-select">
        ...
        <div class="sidebar-section">
            ...
            <button class="sidebar-select" style="cursor: pointer;">New Analysis</button>
            <button class="sidebar-select" style="cursor: pointer;">View History</button>
            <button class="sidebar-select" style="cursor: pointer;">Settings</button>
        </div>
        
        <div class="sidebar-section">
            <div class="sidebar-section-title">Medical Disclaimer</div>
            <p style="font-size: 0.8rem; color: var(--text-light);">
                This tool assists medical professionals only. Final diagnosis and treatment decisions must be made by qualified physicians.
            </p>
        </div>
    """, unsafe_allow_html=True)
...
st.markdown("""
    <div style="background: linear-gradient(135deg, var(--primary), var(--primary-dark));
        ...
        <h1 style="margin-bottom: 0.5rem;">BoneScan AI Medical System</h1>
        <h3 style="font-weight: 400; margin-top: 0;">Advanced X-ray Analysis & Prescription Tool</h3>
    </div>
""", unsafe_allow_html=True)
    ...
    with col1:
        ...
        if uploaded_file:
            try:
                ...
                # Analyze button
                if st.button("Analyze X-ray", key="analyze_btn", type="primary"):
                    ...
                    with st.spinner("Analyzing X-ray..."):
                        ...
                        with st.expander("Generate Medical Report", expanded=True):
                                ...
                                if st.form_submit_button("Generate Report", type="primary"):
                                    if not all([patient_name, patient_age, patient_id, doctor_name, doctor_specialty, doctor_license]):
                                        ...
                                        with st.spinner("Generating report..."):
                                            ...
                                            pdf_bytes = create_medical_report(
    ...
    with col2:
        st.markdown(f"""
            <div class="card model-card">
                ...
                <p>Performance metrics for selected models:</p>
                ...
                <div style="margin: 1rem 0;">
                    ...
                    <div style="height: 6px; background: var(--border); border-radius: 3px; margin: 0.3rem 0;">
                        <div style="width: 91%; height: 100%; background: var(--primary); border-radius: 3px;"></div>
                    </div>
                </div>
...
with tab2:
    ...
    with st.form("prescription_form"):
        ...
        submitted = st.form_submit_button("Generate Prescription", type="primary")
        ...
        if submitted:
            if not all([patient_name, patient_age, patient_id, diagnosis, doctor_name, doctor_specialty, doctor_license]):
                ...
                with st.spinner("Generating prescription..."):
                    ...
                    st.markdown(create_download_button(pdf_bytes, "Medical_Prescription.pdf"), unsafe_allow_html=True)
...
# JavaScript to sync sidebar model selection with main content
st.markdown("""
<script>
// Sync sidebar model select with main content
const sidebarSelect = document.getElementById('sidebar-model-select');
const mainSelect = document.querySelector('select[aria-label="🧠 Select AI Model"]');

if (sidebarSelect && mainSelect) {
    ...
    mainSelect.addEventListener('change', function() {
        sidebarSelect.value = this.value;
    });
}
</script>
""", unsafe_allow_html=True)
